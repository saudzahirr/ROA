...
%% -------------------------------
%  Fast Decoupled Sweep
% -------------------------------

% For 2-bus system, the simplified H and N matrices:
B_prime = imag(Y_bus(1,2)); % dP/dθ approx = B' (off-diagonal)
B_prime_mat = B_prime;      % 1x1 for PQ bus
B_double_prime = -imag(Y_bus(2,2)); % dQ/dV approx = B'' (diagonal)
B_double_prime_mat = B_double_prime; % 1x1 for PQ bus
...
for i = 1:nV
    for j = 1:nd
        ...
        while iter < max_iter
            ...
            % Fast Decoupled: approximate Jacobians
            dtheta = dP / B_prime_mat;
            dV     = dQ / B_double_prime_mat;

            % Update PQ bus
            delta(pq_bus_id) = delta(pq_bus_id) + dtheta;
            V(pq_bus_id)     = V(pq_bus_id) + dV;
...
ylabel('v_2 [pu]')
if addTitle
    title('Region of Attraction of Fast Decoupled Method')
end
...
saveas(gcf, 'FastDecoupled-RegionOfAttraction.png');
