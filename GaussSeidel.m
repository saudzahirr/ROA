...
tolerance = 1e-5;
max_iter = 25;   % Gauss–Seidel usually slower than NR
...
for i = 1:nV
    for j = 1:nd
        ...
        while iter < max_iter
            ...
            % V_k = (1/Y_kk) * [ (S_k*/V_k_old*) - sum_{m≠k} (Y_km * V_m) ]
            sum_term = 0;
            for m = 1:n_bus
                if m ~= k
            ...
            V(k) = (1/Y_bus(k,k)) * ( (conj(S_spec(k))/conj(Vk_old)) - sum_term );

            % Check convergence (mismatch in |ΔV|)
            if abs(V(k) - Vk_old) < tolerance
                converged = true;
                break;
            end
        end
...
ylabel('v_2 [pu]')
if addTitle
    title('Region of Attraction of Gauss-Seidel Method')
end
...
saveas(gcf, 'GaussSeidel-RegionOfAttraction.png');
